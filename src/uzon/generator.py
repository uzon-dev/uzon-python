...
class _Generator:
    ...
    def emit_bindings(self, d: dict[str, Any], level: int) -> str:
        ...
        for key, val in d.items():
            ...
            val_str = self._emit_value(val, level)
    ...
    def _emit_name(self, name: str) -> str:
        ...
        if name in ALL_KEYWORDS:
            return f"@{name}"
        # §2.3: Special characters need quoting
        if any(c in _NEEDS_QUOTING for c in name):
            escaped = name.replace("\\", "\\\\").replace("'", "\\'")
            return f"'{escaped}'"
        return name

    def _emit_value(self, val: Any, level: int) -> str:
        """Emit a UZON value."""
        if val is None:
            return "null"
        ...
        if isinstance(val, UzonEnum):
            return self._emit_enum(val)
        if isinstance(val, UzonTaggedUnion):
            return self._emit_tagged_union(val, level)
        if isinstance(val, UzonUnion):
            return self._emit_union(val, level)
    ...
    def _emit_list(self, lst: list[Any], level: int) -> str:
        """§3.4: Emit list literal."""
        if not lst:
            return "[]"
        parts = [self._emit_value(v, level) for v in lst]
        one_line = "[ " + ", ".join(parts) + " ]"
        if len(one_line) <= 80 and "\n" not in one_line:
            return one_line
        prefix = " " * (self._indent * (level + 1))
        inner_prefix = " " * (self._indent * level)
        inner_parts = [f"{prefix}{self._emit_value(v, level + 1)}" for v in lst]
        return "[\n" + ",\n".join(inner_parts) + "\n" + inner_prefix + "]"
    ...
    def _emit_tuple(self, t: tuple[Any, ...], level: int) -> str:
        ...
        if not t:
            return "()"
        if len(t) == 1:
            return f"({self._emit_value(t[0], level)},)"
        parts = [self._emit_value(v, level) for v in t]
        one_line = "(" + ", ".join(parts) + ")"
        if len(one_line) <= 80 and "\n" not in one_line:
            return one_line
        prefix = " " * (self._indent * (level + 1))
        inner_prefix = " " * (self._indent * level)
        inner_parts = [f"{prefix}{self._emit_value(v, level + 1)}" for v in t]
        return "(\n" + ",\n".join(inner_parts) + "\n" + inner_prefix + ")"

    # ── variant types ─────────────────────────────────────────────

    def _emit_enum(self, val: UzonEnum) -> str:
        """§3.5: Emit enum value."""
    ...
    def _emit_union(self, val: UzonUnion, level: int) -> str:
    ...
    def _emit_tagged_union(self, val: UzonTaggedUnion, level: int) -> str:
        """§3.7: Emit tagged union value."""
        inner = self._emit_value(val.value, level)
        esc = self._escape_variant
